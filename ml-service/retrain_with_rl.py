...
# Load Mongo URI
# NOTE: path fixed to sit next to this file (was "backend/db/.env", a folder that
# never existed in the original repo) so it actually finds the ml-service/.env
# copied over from churn-app/db/.env. No RL/model logic touched.
dotenv_path = Path(__file__).resolve().parent / ".env"
load_dotenv(dotenv_path)
mongo_uri = os.getenv("MONGO_URI", "mongodb://localhost:27017")

# MongoDB connection
# NOTE: was hardcoded to db = client["churn_prediction"]; now takes the
# database named in MONGO_URI itself, same fix as predict.py, so retraining
# reads the same wrong_predictions data the live app just wrote.
...
# === Visualizations ===
# NOTE: made relative to this file instead of the process cwd (was "../outputs",
# which only worked if the script happened to be launched from inside backend/).
# Same q_table/rewards plots, same folder name, just a portable path.
